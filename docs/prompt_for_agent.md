# Prompt for Generating Candi Design System Source

You are an expert design system architect and JavaScript developer. Your task is to generate the `src/` directory for the **Candi Design System**, a multi-platform design system based on Nordic/Scandinavian design principles: **Hygge** (warmth/coziness) and **Lagom** (balance/appropriateness).

## Core Requirements

1. **Single Source of Truth**: All color definitions must originate from a single file: [src/data/colors.js](file:///home/anubhav/src/gh/wtasg/candi/src/data/colors.js).
2. **Color Space**: Use **OKLCH** for all color definitions to ensure perceptual uniformity and accessibility.
3. **Tailwind CSS Integration**: The system must provide a seamless integration with Tailwind CSS via a plugin and theme extension.
4. **No Hex Codes**: Do not use hex codes for colors. Use OKLCH function strings (e.g., `oklch(98% 0.008 85)`).
5. **Data Structure**: Colors must be defined with metadata (`name`, `usage`) to support documentation generation.
