...
function testVimTheme() {
    try {
        ...
        assert.equal(guifg, '#E0DEDA', 'Normal foreground mismatch');
        assert.equal(guibg, '#171614', 'Normal background mismatch');
        assert.ok(darkContent.includes('hi CursorLine') && darkContent.includes('guibg=#252421'), 'CursorLine mapping mismatch');
        assert.ok(darkContent.includes('hi Function') && darkContent.includes('guifg=#3D98D1'), 'Function mapping mismatch');
        assert.ok(darkContent.includes('hi StatusLine') && darkContent.includes('guifg=#E0DEDA') && darkContent.includes('guibg=#30597B'), 'StatusLine mapping mismatch');
        ...
        assert.ok(lightContent.includes('hi Normal') && lightContent.includes('guifg=#0D0B07') && lightContent.includes('guibg=#F8F6F3'), 'Light Normal highlight mapping mismatch');
        assert.ok(lightContent.includes('hi StatusLine') && lightContent.includes('guifg=#FFFFFF') && lightContent.includes('guibg=#0068A7'), 'Light StatusLine mapping mismatch');
        assert.ok(lightContent.includes('hi User1') && lightContent.includes('guifg=#FFFFFF') && lightContent.includes('guibg=#0068A7'), 'Light User1 mapping mismatch');
        assert.ok(lightContent.includes('hi Airlinea') && lightContent.includes('guifg=#FFFFFF') && lightContent.includes('guibg=#0068A7'), 'Light Airlinea mapping mismatch');
        assert.ok(lightContent.includes('hi Airlineb') && lightContent.includes('guifg=#0D0B07') && lightContent.includes('guibg=#F1EEE9'), 'Light Airlineb mapping mismatch');
